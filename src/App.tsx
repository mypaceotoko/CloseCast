...
const recommendItems = [
  '普通のSNSでは話しにくいことを、近い人にだけ残したい人',
  '相互フォローの相手だけに声を届けたい人',
  '仲間内で近況や気持ちを共有したい人',
  'ポッドキャストほど大げさではない音声投稿をしたい人',
  '家族や親しい人に声の記録を残したい人',
  '創作仲間・活動仲間とクローズドにやりとりしたい人',
  'テスターコミュニティで気軽に使ってみたい人'
];

const comparisons = {
  normal: ['広く届く', '見られすぎる', 'バズや数字を意識しやすい', '気軽な本音を出しにくい'],
  friendcast: ['相互フォローの相手だけに届く', '声で温度感が伝わる', '声の返信で会話できる', '通話より軽く、SNSより安心']
};
...
const features = [
  ...
  ['🧭', '公開範囲設定', '相互フォロー、フォロワー、自分のみ、カスタムなどを選べます。'],
  ['🔖', 'あとで聴く', '気になる声はしおり保存して、落ち着いたタイミングで聴けます。']
];
      ...
      <main className="container">
        <section className="hero section" aria-labelledby="hero-title">
          <div className="hero-left fade-in">
            ...
            <div className="badge-row" aria-label="friendcastの特徴">
              {['相互フォロー制', '音声投稿・返信', 'あとで聴く', '公開範囲を選べる'].map((b) => (
                <span key={b} className="badge">{b}</span>
              ))}
            </div>
            <div className="cta-row">
              <a
                aria-label="friendcastを開く（新しいタブで開きます）"
              ...
              <div className="post-card">
                ...
                <div className="voice-card">
                  <div className="player-row">
                    <button className="play" tabIndex={-1} aria-label="再生">▶</button>
                  ...
                  <div className="meta-row"><span>0:18 / 2:26</span><button className="speed" tabIndex={-1}>1.2x</button></div>
                ...
                <div className="actions" aria-label="投稿アクション">
                  <span>💬 コメント 2</span>
                  <span>♡ いいね 4</span>
                  <span><i className="bookmark-icon" />あとで聴く</span>
                </div>
              </div>
              <div className="post-card mini-card">
                <div className="reply-label"><span>↩</span> 音声返信</div>
                <div className="reply-mini-player">
                  <button className="mini-play" tabIndex={-1}>▶</button>
                  <div className="wave mini-wave">{Array.from({ length: 10 }).map((_, i) => <span key={i} style={{ animationDelay: `${i * 0.08}s` }} />)}</div>
                  <i className="bookmark-icon saved" />
                </div>
              </div>
              <div className="bottom-nav"><span>ホーム</span><span>録音</span><span>通知</span><span>マイページ</span></div>
            </div>
          </div>
        </section>

        <section className="section voice-reply card" aria-labelledby="voice-reply-title">
          <div className="voice-reply-head">
            <p className="voice-reply-kicker">VOICE REPLY</p>
            <h2 id="voice-reply-title">返信も、声で返せる。</h2>
            <p>テキストだけでは伝わりにくい温度感も、声ならそのまま届く。friendcastなら、投稿だけでなく返信も音声で残せます。</p>
          </div>
          <div className="voice-reply-layout">
            ...
            <div className="voice-reply-visual" aria-hidden="true">
              ...
              <div className="reply-player">
                ...
                <div className="wave">{Array.from({ length: 12 }).map((_, i) => <span key={i} style={{ animationDelay: `${i * 0.1}s` }} />)}</div>
              </div>
              <div className="reply-bubble outgoing">ありがとう！声で返すね。</div>
            </div>
          </div>
        </section>
        ...
        <section className="section" id="features" aria-labelledby="features-title">
          <div className="section-head">
            ...
            <p>声で残す、声で返す、文章で補う、届ける相手を選ぶ。friendcastの基本機能です。</p>
          </div>
          <div className="grid">
        ...
        <section className="section" aria-labelledby="scenes-title">
          <div className="section-head">
            ...
            <p>個人の近況から、小さなコミュニティのやりとりまで。広く見せる前提ではない声の置き場所です。</p>
          </div>
        ...
        <section className="section card updates-card" aria-labelledby="updates-title">
          ...
          <p>friendcastは現在開発中です。テスターのフィードバックを受けながら、声でつながる体験を少しずつ改善しています。</p>
          <ul className="updates">
            {roadmapItems.map(([status, item]) => (
              <li key={item}><span>{status}</span>{item}</li>
            ))}
          </ul>
        </section>
        ...
        <section className="section cta card" aria-labelledby="cta-title">
          ...
          <p>普通のSNSに疲れた人が、友達や仲間と安心して声でつながれる場所をつくっています。投稿も返信も声でできるクローズドな場所を、まずはテスターとして触ってみてください。</p>
          <div className="cta-row">
            <a
              className="btn primary"
              href={appUrl}
